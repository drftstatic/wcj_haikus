import type { Haiku, HaikuOptions } from './generator';

const HASHTAG_LOOKUP: {
    shows: { [key: string]: string };
    eras: { [key: string]: string };
    wrestlers: { [key: string]: string };
} = {
    shows: {
        'RAW': '#WWERAW',
        'SmackDown': '#SmackDown',
    ...
    // A small, curated list of wrestler hashtags
    wrestlers: {
...
export function buildHashtags(inputs: HaikuOptions): string[] {
...
export function buildTweetText(haiku: Haiku, meta: HaikuOptions, hashtags: string[], format: 'compact' | 'full' = 'full'): string {
    const MAX_LEN = 280;
