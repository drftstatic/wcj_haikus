...
function getRandomElement(arr, randomFn) {
    // This function now assumes it will not be called with an empty array.
    // The caller is responsible for the check.
...
export function generateHaiku(options, lexicon) {
    ...
    if (requestedWrestlers && requestedWrestlers.length > 0) {
        relevantWrestlers = lexicon.wrestlers.filter(w => requestedWrestlers.some(optW => w.name.toLowerCase().includes(optW.toLowerCase())));
    }
    else if (options.era) {
