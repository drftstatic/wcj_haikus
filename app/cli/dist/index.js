...
import { generateHaiku } from './utils/generator.js';
...
async function main() {
  ...
  yargs(hideBin(process.argv))
    ...
    .command('$0', 'Generate a wrestling haiku', () => {}, async (argv) => {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (argv.shareX) {
            ...
            output.tweetText = buildTweetText(haiku, options, hashtags, argv.shareX);
        }
    ...
    .option('wrestlers', { alias: 'w', description: 'Wrestler names to influence the haiku' })
    .option('era', { alias: 'e', description: 'Era to influence the haiku (e.g., "Attitude")' })
    .option('show', { alias: 's', description: 'Show or PPV to influence the haiku (e.g., "RAW")' })
    .option('match', { alias: 'm', description: 'Match type to influence the haiku (e.g., "Ladder")' })
    .option('tone', { alias: 't', description: 'The tone of the haiku' })
    .option('chaos', { description: 'Chaos level (0-10)', default: 0 })
    .option('seed', { description: 'Seed for the random number generator for deterministic output' })
    .option('count', { alias: 'c', description: 'Number of haikus to generate', default: 1 })
    .option('json', { description: 'Output in JSON format', default: false })
    .option('hashtags', { description: 'Generate hashtags', default: true })
    .option('share-x', { description: 'Output tweet-ready text' })
    .help()
    .argv;
}
